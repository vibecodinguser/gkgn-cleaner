...
def clean_dmm_columns_5_and_6(df: pd.DataFrame) -> pd.DataFrame:
    ...
    results = [_clean_col5_col6_dmm(out.iat[i, 4], out.iat[i, 5]) for i in range(len(out))]
    new5_list, new6_list = zip(*results) if results else ([], [])
    out.iloc[:, 4] = new5_list
    out.iloc[:, 5] = new6_list
    return out
...
def clean_column_7_sheet_code(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = df.copy()
    col = out.iloc[:, 6]
    out.iloc[:, 6] = col.map(_extract_sheet_code_x_xx_xxx)
    return out
